...
<template>
  <div class="h-full flex flex-col bg-white rounded-xl shadow-sm overflow-hidden">
    ...
    <div class="flex-1 overflow-y-auto">
      <div v-if="workspaces.length === 0" class="flex flex-col items-center justify-center h-full py-12 px-4 text-gray-400">
        <svg class="w-12 h-12 mb-3 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
        </svg>
        <p class="text-sm text-center">暂无工作区</p>
        <p class="text-xs text-center mt-1">上传音频文件自动创建</p>
      </div>
